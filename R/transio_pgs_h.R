...
transio_pgs_h<-function(pF1,pF2,a,b,r1,r2,rho,shift,h){
  ...
  t11<- pvmnorm(lower=c(-Inf,-Inf),upper=c(a,a),mean = c(0,0),sigma=matrix(c(1,rho1h,rho1h,1),2,2))/pF1
  sigma <- matrix(c(1,rho1h,rho12h,rho1h,1,rho,rho12h,rho,1),3,3)
  t12<- util.mvncdf(lower=c(-Inf,a,b),upper=c(a,Inf,Inf),mean = c(0,0,0),sigma=sigma)/pF1
  ...
  t22<- util.mvncdf(lower=c(a,a,b,b),upper=c(Inf,Inf,Inf,Inf),mean = c(0,0,0,0),sigma=sigma)/pF2
