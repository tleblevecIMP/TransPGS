...
monte_carlo<-function(ncell,n,xdim,dist_max,pF1,pF2,rho,r1,r2,shift){
  ...
  # the range in rgeostat is multiplied by sqrt(3)
  model1<-model.create(ndim=2,nvar=1,vartype=4,aniso.coeffs=c(1,1),range=sqrt(3)*r1)
  model2<-model.create(ndim=2,nvar=1,vartype=4,aniso.coeffs=c(1,1),range=sqrt(3)*r2)
